...
def load_data_h5py(NAME, fl):
    if NAME in fl:
        with h5py.File('data/'+NAME, 'r') as f:
            ...
            ###             'samples', 'seed', 'session', 'sessionTypes', 'sr' = 2000, 
            ###             'tooshort'])
...
def compile_data(dfs, cond, sess, amount_data, band, l_or_r = 'l', 
                  trial_num = 0, se = [0, 0]):
    t_name = 'data_' + l_or_r

    if amount_data =='single_trial':
        t_data = dfs[t_name + '_' + str(trial_num)]
        band_array, smp_len = calc_band_pwr(t_data, band)
        if l_or_r == 'l':
            band_array['trial_condition'] = [cond[trial_num]]*smp_len
        elif l_or_r == 'r':
            if cond[trial_num] == 2:
                band_array['trial_condition'] = [3]*smp_len
            elif cond[trial_num] == 3:
                band_array['trial_condition'] = [2]*smp_len
            else: 
                band_array['trial_condition'] = [cond[trial_num]]*smp_len
        band_array['session'] = [sess]*smp_len
        t_length = 'Single trial'
    
    elif amount_data =='part_trial':
        t_data = dfs[t_name + '_' + str(trial_num)].iloc[se[0]:se[1]]
        band_array, smp_len = calc_band_pwr(t_data, band)
        if l_or_r == 'l':
            band_array['trial_condition'] = [cond[trial_num]]*smp_len
        elif l_or_r == 'r':
...
def apply_mapper(trial_data, proj_func, clus_func, num_cubes, perc_overlap, param_dict):
    ...
    if clus_func == 'dbscan':
        ...
        print('Option not available.')
    cov = km.Cover(num_cubes, perc_overlap)
    ### Apply mapper
    mapper = km.KeplerMapper(verbose=2)
    data_4_mapper = trial_data.iloc[:, 0:32].to_numpy()
    ...
    print("Output graph examples to html with parameters in txt")
    ...
    mapper.visualize(graph, 
                      title = param_dict['tl'] + ' Subject ' + param_dict['lr'], 
                      path_html = 'Figures/mapper_outputs/' + str(dt) + ".html",
                      color_values = col_val,
                      color_function_name = 'Trial Type')
    ### Save trial info
    graph_data = 'Figures/mapper_outputs/' + str(dt) + '_dict' + '.pkl'
    with open(graph_data, 'wb') as f:
        pickle.dump(graph, f)
    text = open('Figures/mapper_outputs/' + str(dt) + '.txt', 'x')
    stats = ['Trial: ', str(param_dict['tn']), '\n',
              'Session: ', str(param_dict['tt']), '\n',
              'Left or right: ', str(param_dict['lr']), '\n',
              'Full, part, or concat dataset: ', str(param_dict['ad']), '\n',
              'If part, trial start and end: ', str(param_dict['se']), '\n',
              'Mapper projection algorithm: ', str(proj), '\n',
              'Mapper clustering algorithm: ', str(clus), '\n',
              'Number of hypercubes: ', str(num_cubes), '\n',
              'Percent overlap: ', str(perc_overlap), '\n'
              ]
    text.writelines(stats)
    text.close()
    return graph_data
...
#%% Plot the synchronize/syncopated graph
def make_pie_graph(nx_graph, tdt, loaded_graph, fsize, piesize, l_or_r, ss_or_four):
    ...
    plt.title('Four Conditions, ' + l_or_r, loc = 'left', fontsize =30)
    
    if ss_or_four == 'ss':
    ### Plot legend (synchro vs synco)
        pi_patch = mpatches.Patch(color='pink', label='Synchronized')
        bl_patch = mpatches.Patch(color='deepskyblue', label='Syncopated')
        plt.legend(handles=[pi_patch, bl_patch], fontsize = 30)
    elif ss_or_four == 'four':
...
def calc_qmod(loaded_graph, nx_graph, trial_data, ss_or_four):
    ...
    community_labels = []
    ### For four trial-based Qmod
    cond_np = trial_data['trial_condition'].to_numpy()
    ### For s/s-based Qmod
    sess_np = trial_data['session'].to_numpy()
    for key in nx_graph:
        current_cluster_pts = loaded_graph['nodes'].get(key)
        current_labels = []
        ...
        for l, x in enumerate(fracs):
            if x == max(fracs):
                m_list.append(l+1)
        if len(m_list) == 1:
            community_labels.append(m_list[0])
        else:
            community_labels.append(random.choice(m_list))
### Note, I went with random for now. 
    qmod = []
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            Ci = community_labels[i]
            Cj = community_labels[j]
            if Ci == Cj and m != 0:
                Aij = A[i,j]
                ki = np.sum(I[[i],:])
                kj = np.sum(I[[j],:])
