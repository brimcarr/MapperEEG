...
#%% GMM
            ...
            for comp in range(2,11):
                for ind, cov in enumerate(cov_types):
                    print(comp, cov)
                    gmm_labels = GaussianMixture(n_components = comp, covariance_type=cov).fit_predict(t_data)+1
                            
                    errs = calc_errors(t_data, trial_labels, gmm_labels)
                    gmm_stat_array.append([comp, ind, errs[0], errs[1], errs[2], errs[3], 
                                            errs[4], errs[5]])
        parent.append(gmm_stat_array) 
       

#%% Make error array
parent_np = np.array(parent)
pnp = np.hstack(parent_np)
np.savetxt('errors/all_errors_GMM.csv', pnp, delimiter=',')        
