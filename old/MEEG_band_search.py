        ...
        for bd in band_dict.keys():
            ...
            for perc in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]:
                for cube in [10, 15, 20, 25, 30, 35, 40]:
                    ...
                    graph_data = apply_mapper(trial_data, proj_func, clus_func,  
                                              num_cubes, perc_overlap, param_dict)
                    ...
                    make_pie_graph(nx_graph, trial_data, loaded_graph, fsize, piesize,
                                      ' Tapper: ' + lr + ' Overlap: ' + str(perc) 
                                      + ' Num of Cubes: ' + str(cube) + ' Band: ' 
                                      + str(band), ss_or_four = 'four')
                    make_pie_graph(nx_graph, trial_data, loaded_graph, fsize, piesize,
                                      ' Tapper: ' + lr + ' Overlap: ' + str(perc) 
                                      + ' Num of Cubes: ' + str(cube) + ' Band: ' 
                                      + str(band), ss_or_four = 'ss')
...
np.savetxt('Qmod/' + 'band_qmod_four_24feb2025.csv', np_qmod_four, delimiter=',')
np.savetxt('Qmod/' + 'band_qmod_ss_24feb2025.csv', np_qmod_ss, delimiter=',')
