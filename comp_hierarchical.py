...
nec_array = []
nec_header = ['Dyad', 'lr', '# of clusters', 'f1_micro', 'f1_macro', 'f1_weight', 'sil_score', 'l_loss', 'db_score'] 
#%% Load up the data and choose trial
### Pull all datafile names
#%% Run k-means
def comp_hierarchical(trial_data, trial_labels, p_d, nec_array, csv_list):
    error_array = []
    t_data = trial_data.iloc[:,0:32]
    for clus in p_d['hp_1']:
        agg_clustering = AgglomerativeClustering(n_clusters=clus)
        agg_labels = agg_clustering.fit_predict(t_data).tolist()
        prop_labels = make_proper_labels(agg_labels, trial_labels) 
...
for leftright in ['l', 'r']:
    ...
    for i in range(6):
        ...
        nec_arr = comp_hierarchical(t_d, t_l, p_d, nec_array, csv_list)
